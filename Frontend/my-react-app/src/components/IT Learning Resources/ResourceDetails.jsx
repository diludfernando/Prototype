...
import {
    ...
    Heart, Share2, CheckCircle, MessageSquare, StickyNote,
        ...
        try {
            const res = await fetch('http://localhost:8084/api/enrollments', {
                ...
                body: JSON.stringify({
                    ...
                    purchaseProofUrl: purchaseData.purchaseProofUrl || null,
            ...
            if (res.ok) {
                ...
                showToastMsg('Course added to My Courses! Start tracking your progress.');
            }
        } catch (err) { console.error(err); }
    };
                ...
                <div className="modal-overlay" onClick={() => setShowPurchaseForm(false)}>
                    <div className="modal" onClick={e => e.stopPropagation()}>
                        ...
                        <form onSubmit={handlePurchaseSubmit}>
                            <div className="modal-body">
                                <div className="modal-helper">
                                    You purchased this course on <strong>{course.provider}</strong>. Add it to SkillBridge to track progress and submit your certificate.
                                </div>
                                <div>
                                    <label className="modal-label">External Course URL <span className="required-mark">*</span></label>
                                    <input
                                        className="modal-input"
                                        type="url"
                                        placeholder={`e.g. https://www.${course.provider?.toLowerCase()}.com/course/...`}
                                        value={purchaseData.purchaseProofUrl}
                                        onChange={e => setPurchaseData({ ...purchaseData, purchaseProofUrl: e.target.value })}
                                        required
                                    />
                                    <p className="modal-meta">Paste your course link from {course.provider}</p>
                                </div>
                                <div>
                                    <label className="modal-label">Order / Reference ID <span className="required-mark">*</span></label>
                                    <input
                                        className="modal-input"
                                        type="text"
                                        placeholder="e.g. UC-XXXXXXXX or order #12345"
                                        value={purchaseData.orderId}
                                        onChange={e => setPurchaseData({ ...purchaseData, orderId: e.target.value })}
                                        required
                                    />
                                    <p className="modal-meta">Found in your purchase confirmation email</p>
                                </div>
                            </div>
                            <div className="modal-footer">
                                <button type="submit" className="btn-primary">
                                    <CheckCircle size={15} />&nbsp;Add to My Courses
                                </button>
                            </div>
                        </form>
                    </div>
            ...
            <div className="container">
                ...
                <div className="details-grid">
                    ...
                    <div className="details-main">
                        ...
                        {/* Progress Tracker — enrolled only */}
                        {isEnrolled && (
