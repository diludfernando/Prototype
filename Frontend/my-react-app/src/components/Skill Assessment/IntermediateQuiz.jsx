...
const IntermediateQuiz = () => {
    ...
    useEffect(() => {
        const fetchIntermediateQuestions = async () => {
            try {
                const response = await fetch(`http://localhost:8082/api/questions?level=Medium&category=${category}`);
                if (!response.ok) throw new Error('Failed to fetch intermediate questions');
                const data = await response.json();

                if (data.length === 0) {
                    setError(`No intermediate level questions for ${category} available at the moment.`);
                } else {
    ...
    const handleQuizCompletion = async (finalScore) => {
        ...
        if (percentage === 100) {
            localTitle = "Intermediate Master!";
            localMessage = "Outstanding! You've navigated the complexities of this level with ease. Ready for the Advanced stage?";
        } else if (percentage >= 80) {
            localTitle = "Solid Professionalism!";
            localMessage = "Great progress. You clearly understand the core principles and are well on your way to mastery.";
        } else if (percentage >= 50) {
            localTitle = "Competent Performer!";
            localMessage = "A respectable score. Focus on some of the trickier edge cases to reach the next level.";
        } else {
            localTitle = "Building Capacity";
            localMessage = "You're getting there. Review your areas of improvement and target them in your next attempt.";
        }
    ...
    const getResultIcon = () => {
        ...
        if (percentage === 100) return <Award size={80} color="#0ea5e9" />;
        if (percentage >= 80) return <Trophy size={80} color="#0ea5e9" />;
        if (percentage >= 50) return <Star size={80} color="#0ea5e9" />;
        return <RotateCcw size={80} color="#64748b" />;
    };

    if (loading) {
        return (
            <div className="intermediate-quiz-page">
    ...
    return (
        <div className="intermediate-quiz-page typography-inter">
            {isSavingResult ? (
                <div className="quiz-premium-card loading text-center">
                    <div className="loader mx-auto mb-4"></div>
                ...
                <div className="quiz-layout-wrapper">
                    ...
                    {/* Centered Quiz Card */}
                    <div className="quiz-premium-card redesigned-card pro-border">
